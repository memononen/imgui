...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)
{
    ...
    if (hovered)
        window->DrawList->AddRectFilled(bb.Min, bb.Max, bg_col, 0.f);
...
bool ImGui::ImageButtonEx(ImGuiID id, ImTextureRef tex_ref, const ImVec2& image_size, const ImVec2& uv0, const ImVec2& uv1, const ImVec4& bg_col, const ImVec4& tint_col, ImGuiButtonFlags flags)
{
    ...
    if (bg_col.w > 0.0f)
        window->DrawList->AddRectFilled(bb.Min + padding, bb.Max - padding, GetColorU32(bg_col), 0.f);
...
bool ImGui::TextLink(const char* label)
{
    ...
    window->DrawList->AddHorizontalLine(bb.Min.x, bb.Max.x, line_y, GetColorU32(line_colf), ImDrawFlags_None, 1.0f * (float)(int)g.Style._MainScale); // FIXME-TEXT: Underline mode // FIXME-DPI
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, float thickness)
{
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        // Draw
        window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImGuiCol_Separator), 0);
        if (g.LogEnabled)
            LogText(" |");
    }
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        {
            // Draw
            window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(ImGuiCol_Separator), 0.f);
            if (g.LogEnabled)
...
void ImGui::SeparatorTextEx(ImGuiID id, const char* label, const char* label_end, float extra_w)
{
    ...
    if (label_size.x > 0.0f)
    {
        ...
        if (sep1_x2 > sep1_x1 && separator_thickness > 0.0f)
            window->DrawList->AddHorizontalLine(sep1_x1, sep1_x2, seps_y, separator_col, ImDrawFlags_None, separator_thickness);
        if (sep2_x2 > sep2_x1 && separator_thickness > 0.0f)
            window->DrawList->AddHorizontalLine(sep2_x1, sep2_x2, seps_y, separator_col, ImDrawFlags_None, separator_thickness);
        if (g.LogEnabled)
    ...
    else
    {
        ...
        if (separator_thickness > 0.0f)
            window->DrawList->AddHorizontalLine(sep1_x1, sep2_x2, seps_y, separator_col, ImDrawFlags_None, separator_thickness);
    }
}
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            for (int line_n = line_visible_n0; line_n < line_visible_n1; line_n++)
            {
                ...
                draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color, 0.f);
            }
        }
    }
    ...
    if (render_cursor)
    {
        ...
        if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
            draw_window->DrawList->AddVerticalLine(cursor_screen_rect.Min.x, cursor_screen_rect.Min.y, cursor_screen_rect.Max.y, GetColorU32(ImGuiCol_InputTextCursor), ImDrawFlags_None, 1.0f * (float)(int)style._MainScale); // FIXME-DPI: Cursor thickness (#7031)
...
void ImGui::EndBoxSelect(const ImRect& scope_rect, ImGuiMultiSelectFlags ms_flags)
{
    ...
    window->DrawList->AddRectFilled(box_select_r.Min, box_select_r.Max, GetColorU32(ImGuiCol_SeparatorHovered, 0.30f), 0.f); // FIXME-MULTISELECT: Styling
        ...
        for (int n = 0; n < res_w; n++)
        {
            ...
            else if (plot_type == ImGuiPlotType_Histogram)
            {
                if (pos1.x >= pos0.x + 2.0f)
                    pos1.x -= 1.0f;
                window->DrawList->AddRectFilled(pos0, pos1, idx_hovered == v1_idx ? col_hovered : col_base, 0.f);
            }
...
bool    ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags)
{
    ...
    if (g.Style.TabBarBorderSize > 0.0f)
    {
        ...
        window->DrawList->AddRectFilled(ImVec2(tab_bar->SeparatorMinX, y - g.Style.TabBarBorderSize), ImVec2(tab_bar->SeparatorMaxX, y), col, 0.f);
    }
    return true;
}
