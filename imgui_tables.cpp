...
void ImGui::TableEndRow(ImGuiTable* table)
{
    ...
    if (is_visible)
    {
        ...
        if (top_border_col && bg_y1 >= table->BgClipRect.Min.y && bg_y1 < table->BgClipRect.Max.y)
            window->DrawList->AddHorizontalLine(table->BorderX1, table->BorderX2, bg_y1, top_border_col, ImDrawFlags_None, border_size);

        // Draw bottom border at the row unfreezing mark (always strong)
        if (draw_strong_bottom_border && bg_y2 >= table->BgClipRect.Min.y && bg_y2 < table->BgClipRect.Max.y)
            window->DrawList->AddHorizontalLine(table->BorderX1, table->BorderX2, bg_y2, table->BorderColorStrong, ImDrawFlags_None, border_size);
    }
...
void ImGui::TableDrawBorders(ImGuiTable* table)
{
    ...
    if (table->Flags & ImGuiTableFlags_BordersInnerV)
    {
        for (int order_n = 0; order_n < table->ColumnsCount; order_n++)
        {
            ...
            if (is_frozen_separator)
                ...
                draw_y2 = draw_y2_body;
            if (draw_y2 > draw_y1) {
                inner_drawlist->AddVerticalLine(column->MaxX, draw_y1, draw_y2, TableGetColumnBorderCol(table, order_n, column_n), ImDrawFlags_None, border_size);
            }
        }
    }
    ...
    if (table->Flags & ImGuiTableFlags_BordersOuter)
    {
        ...
        {
            inner_drawlist->AddVerticalLine(outer_border.Min.x, outer_border.Min.y, outer_border.Max.y, outer_col, ImDrawFlags_None, border_size);
            inner_drawlist->AddVerticalLine(outer_border.Max.x - border_size, outer_border.Min.y, outer_border.Max.y, outer_col, ImDrawFlags_None, border_size);
        }
        else if (table->Flags & ImGuiTableFlags_BordersOuterH)
        {
            const float h_inset = (table->Flags & ImGuiTableFlags_BordersOuterV) ? border_size : 0.f;
            inner_drawlist->AddHorizontalLine(outer_border.Min.x + h_inset, outer_border.Max.x - h_inset, outer_border.Min.y, outer_col, ImDrawFlags_None, border_size);
            inner_drawlist->AddHorizontalLine(outer_border.Min.x + h_inset, outer_border.Max.x - h_inset, outer_border.Max.y, outer_col, ImDrawFlags_None, border_size);
        }
    }
    if ((table->Flags & ImGuiTableFlags_BordersInnerH) && table->RowPosY2 < table->OuterRect.Max.y)
    {
        ...
        if (border_y >= table->BgClipRect.Min.y && border_y < table->BgClipRect.Max.y)
            inner_drawlist->AddHorizontalLine(table->BorderX1 + h_inset, table->BorderX2 - h_inset, border_y, table->BorderColorLight, ImDrawFlags_None, border_size);
    }
